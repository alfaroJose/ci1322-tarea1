...
/**
 * Implements the CodeGenerator interface for PostgreSQL DBMS.
 */
public class PostGreSQLCodeGenerator implements CodeGenerator {
    ...
    /**
     * Populate List<String> statements with PostgreSQL statements to create each Entity and its Columns
     */
    @Override
        ...
        for(EntityData ent : interRep) {
            ...
            for(ColumnData col: ent.getColumnDataList()){
                ...
                if(col.isLob())
                    ...
                    if(len == "")
                        len = "255";
                    statement.append(castType(col.getType()) + "(" + len + ")"); //cast to PostgreSQL types
                }
                else
    ...
    /**
     * Populate PostgreSQL script file with statements to create each Entity and its Columns
        ...
        for(EntityData ent : interRep) {
            ...
            for(ColumnData col: ent.getColumnDataList()){
                ...
                if(col.isLob())
                    ...
                    if(len == "")
                        len = "255";
                    script.append(castType(col.getType()) + "(" + len + ")"); //cast to PostgreSQL types
                }
                else
    ...
    /**
     * Returns the PostgreSQL equivalent data type for java types
     * @param type
     * @return newtype
     */
